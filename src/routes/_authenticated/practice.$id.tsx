...
function MissionStage({
  ...
  // Auto-open next step's object after a choice is made
  useEffect(() => {
    if (state.status === "playing" && currentStep) {
      const timer = setTimeout(() => setOpen(currentStep.object), 400);
      return () => clearTimeout(timer);
    }
  }, [state.stepIndex, state.status, currentStep]);
    ...
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden bg-[oklch(0.16_0.025_265)] text-white">
      ...
      <div className="relative z-10 px-4 md:px-6 lg:px-8 py-3">
        <div className="flex flex-wrap items-center gap-3 justify-between rounded-xl bg-black/40 backdrop-blur-md px-4 py-2 border border-white/10 shadow-xl">
          <div className="flex items-center gap-3 min-w-0">
            <Link
              to="/course"
              className="inline-flex items-center text-xs text-white/70 hover:text-white gap-1 shrink-0"
            >
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-[11px] uppercase tracking-[0.18em] text-white/50 leading-none">
                  Симулятор PM · миссия
                </div>
          ...
          <div className="hidden md:flex items-center gap-2 text-[11px] font-mono text-white/60">
            ...
            <span className="px-2 py-1 rounded-md bg-white/5 border border-white/10 inline-flex items-center gap-1.5">
              <SignalHigh className="size-3" /> <Wifi className="size-3" />{" "}
      ...
      <div className="relative z-10 px-3 md:px-6 lg:px-8 pb-6 grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <aside className="rounded-2xl bg-black/45 backdrop-blur-xl border border-white/10 shadow-2xl p-4 space-y-5 self-start lg:sticky lg:top-3">
          ...
          <section className="space-y-3">
            ...
            <ScaleBar
              icon={Heart}
              label="Мотивация команды"
              value={state.scales.morale}
              tone="rose"
            />
            <p className="text-[10px] text-white/40 leading-snug">
              Если любая шкала упадёт до нуля — миссия провалена.
            </p>
          </section>
              ...
              <ul className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
                {state.log
                  ...
                  .map((l, i) => (
                    <li
                      key={i}
                      className="rounded-md bg-white/5 border border-white/10 px-2 py-1.5 text-[11px] leading-snug text-white/70"
                    >
        ...
        <div className="relative">
          ...
          >
            <img
              src={officeBg}
              alt="Интерьер IT-офиса: рабочее место проектного менеджера"
              draggable={false}
            ...
            <button
              ...
              className={cn(
                ...
                activeObject === "dashboard" && "office-bob",
              )}
            ...
            >
              ...
              >
                <div className="relative w-full h-full rounded-sm bg-[oklch(0.98_0.005_90)] overflow-hidden p-[5%] text-[oklch(0.25_0.04_260)]">
                  <div className="font-marker text-[clamp(10px,1.1vw,16px)] uppercase text-[oklch(0.45_0.18_260)]">
                    Цель миссии
                  </div>
                  <div className="font-handwriting font-bold leading-[1.05] mt-1 text-[clamp(13px,1.5vw,24px)]">
                    {mission.goalLabel}
                  </div>
                  <div className="mt-2 font-handwriting text-[clamp(12px,1.4vw,22px)]">
                    ...
                    → цель {mission.metric.target}
                    {mission.metric.unit}
                  </div>
                  {activeObject === "dashboard" && (
                    <span className="absolute inset-0 ring-2 ring-amber-300/70 rounded-sm animate-pulse" />
                  )}
                </div>
                <div className="absolute left-[10%] -bottom-1.5 flex gap-1.5">
                  {["#1f2937", "#2563eb", "#dc2626", "#16a34a"].map((c) => (
                    <span
                      key={c}
                      className="block w-5 h-1.5 rounded-sm shadow-sm"
                      style={{ background: c }}
                    />
                  ))}
                </div>
            ...
            <div className="absolute inset-x-0 bottom-0 h-[46%] pointer-events-none">
              <DeskObject
                style={{ left: "30%", bottom: "0%", width: "40%" }}
                src={laptopImg}
                label="Ноутбук — Zoom и задачи"
                ariaLabel="Открыть ноутбук"
                onClick={() => objectClick("laptop")}
                active={activeObject === "laptop"}
                disabled={!playing}
              />
              <DeskObject
                style={{ right: "10%", bottom: "12%", width: "8.5%" }}
                src={phoneImg}
                label="Телефон — входящие"
                ariaLabel="Открыть телефон"
                onClick={() => objectClick("phone")}
                active={activeObject === "phone"}
                badge={activeObject === "phone" ? "1" : undefined}
                disabled={!playing}
              />
              <DeskObject
                style={{ left: "13%", bottom: "2%", width: "13%" }}
                src={notebookImg}
                label="Папка с документами"
                ariaLabel="Открыть папку с документами"
                onClick={() => objectClick("folder")}
                active={activeObject === "folder"}
                disabled={!playing}
              />
            </div>
          ...
          {playing && currentStep && (
            <div className="mt-3 rounded-xl bg-black/40 backdrop-blur-md border border-primary/30 px-4 py-3 shadow-xl flex items-center gap-3">
              ...
              <Button
                ...
                onClick={() => setOpen(currentStep.object)}
              >
      ...
      {open === "folder" && playing && currentStep?.object === "folder" && (
        <Modal onClose={() => setOpen(null)} label="Папка с документами" wide>
          <FolderPanel
            step={currentStep}
            onChoose={(c) => handleChoice(currentStep, c)}
            metricLabel={mission.metric.label}
          />
        </Modal>
      )}
      ...
      {open && playing && currentStep && open !== currentStep.object && open !== "dashboard" && (
        <Modal onClose={() => setOpen(null)} label={OBJECT_META[open].label} narrow>
          <div className="p-6 text-center text-sm text-muted-foreground">
            Сейчас активен другой объект:{" "}
            <b className="text-foreground">{OBJECT_META[currentStep.object].label}</b>.
            <div className="mt-4">
              <Button size="sm" onClick={() => setOpen(currentStep.object)}>
                Перейти туда
              </Button>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between text-[11px] mb-1">
        ...
        <span
          className={cn(
            ...
            low ? "text-rose-400 font-bold" : "text-white/50",
          )}
        >
          {value}
        </span>
...
function DeskObject({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className="absolute pointer-events-auto group focus:outline-none disabled:opacity-50"
      style={style}
    >
      <div
        className={cn(
          "relative transition-transform duration-300 group-hover:-translate-y-2 group-hover:scale-[1.04]",
          active && "office-bob",
        )}
      >
        <img
          ...
          draggable={false}
          className={cn(
            "w-full h-auto select-none drop-shadow-[0_25px_25px_rgba(0,0,0,0.55)]",
            active && "drop-shadow-[0_0_30px_rgba(99,102,241,0.7)]",
          )}
          style={{ filter: "contrast(1.02)" }}
        />
        {active && (
          <span className="absolute -inset-2 rounded-xl ring-2 ring-primary/70 animate-pulse pointer-events-none" />
        )}
        {badge && (
          <span className="absolute -top-1 -right-1 min-w-6 h-6 px-1.5 rounded-full bg-destructive text-destructive-foreground text-[11px] font-bold grid place-items-center shadow-lg office-notif">
            {badge}
          </span>
        )}
      </div>
  ...
  if (picked) {
    return (
      <div className="space-y-4">
        {zoom && picked.tone && (
          <div className="text-[11px] uppercase tracking-wider text-primary font-semibold">
            Тон: {TONE_LABELS[picked.tone]}
          </div>
        )}
        <div className="rounded-xl bg-secondary px-4 py-3 text-sm">
          <span className="font-semibold">Ты выбрал: </span>
          {picked.text}
        </div>
        {picked.reply && (
          <div className="rounded-xl border bg-card px-4 py-3 text-sm italic text-muted-foreground">
            {picked.reply}
          </div>
        )}
        <div
        ...
        <EffectChips effects={picked.effects} metricLabel={metricLabel} />
  ...
  return (
    <div className="space-y-2.5">
      {choices.map((c) => (
        ...
        >
          {zoom && c.tone && (
            <span className="inline-block mb-1 text-[10px] uppercase tracking-wider text-primary font-semibold">
              {TONE_LABELS[c.tone]}
            </span>
          )}
...
function EffectChips({
  ...
  if (effects.metric) items.push({ label: metricLabel, v: effects.metric });
...
/* ---------- Interactive tasks (drag-to-order / fill-in-the-blank) ---------- */
...
function outcomeToChoice(outcome: Outcome, text: string): Choice {
  return {
    id: "interaction",
    text,
...
function OutcomeView({
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-secondary px-4 py-3 text-sm font-semibold">{label}</div>
      {outcome.reply && (
        <div className="rounded-xl border bg-card px-4 py-3 text-sm italic text-muted-foreground">
          {outcome.reply}
        </div>
      ...
      <EffectChips effects={outcome.effects} metricLabel={metricLabel} />
...
function InteractionView({
  ...
  if (interaction.kind === "order")
    return <OrderTask interaction={interaction} onChoose={onChoose} metricLabel={metricLabel} />;
  return <FillTask interaction={interaction} onChoose={onChoose} metricLabel={metricLabel} />;
}
...
function OrderTask({
  ...
  const [order, setOrder] = useState<string[]>(() => shuffle(interaction.items.map((i) => i.id)));
  const [dragIdx, setDragIdx] = useState<number | null>(null);
  const [resolved, setResolved] = useState<Outcome | null>(null);
  const byId = Object.fromEntries(interaction.items.map((i) => [i.id, i.text]));
  ...
  function check() {
    const correct = order.every((id, i) => id === interaction.correctOrder[i]);
    setResolved(correct ? interaction.success : interaction.fail);
  }
  ...
  if (resolved)
    return (
      <OutcomeView
        outcome={resolved}
        label="Порядок зафиксирован"
        metricLabel={metricLabel}
        onContinue={() => onChoose(outcomeToChoice(resolved, "Порядок шагов"))}
      />
    );
  ...
  return (
    <div>
      <div className="text-[11px] uppercase tracking-wider text-primary font-semibold mb-1">
        Перетаскиванием расставь по порядку
      </div>
      <p className="text-sm text-muted-foreground mb-3">{interaction.prompt}</p>
      <ul className="space-y-2">
        {order.map((id, i) => (
          <li
            ...
            className={cn(
              "flex items-center gap-3 rounded-xl border bg-card px-3 py-2.5 text-sm cursor-grab active:cursor-grabbing",
              dragIdx === i && "opacity-60 border-primary",
            )}
          >
...
function FillTask({
  ...
  function pick(opt: string) {
    setPicked(opt);
    setResolved(opt === interaction.answer ? interaction.success : interaction.fail);
  }
  if (resolved)
    return (
      <OutcomeView
        outcome={resolved}
        label={`Ты вставил: «${picked}»`}
        metricLabel={metricLabel}
        onContinue={() => onChoose(outcomeToChoice(resolved, picked ?? ""))}
      />
    );
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-3">{interaction.prompt}</p>
      <div className="rounded-xl border bg-secondary/40 p-4 text-sm leading-relaxed">
        {interaction.before}{" "}
        <span className="inline-block min-w-16 border-b-2 border-dashed border-primary/60 px-2 text-center font-semibold text-primary">
          {picked ?? "…"}
        </span>{" "}
        {interaction.after}
      </div>
...
function DashboardPanel({
  ...
  mission: NonNullable<ReturnType<typeof getMission>>;
  state: ReturnType<typeof useMissionEngine>["state"];
  metricPct: number;
}) {
  return (
    <div className="p-5 sm:p-6 space-y-4">
      <div>
        <div className="text-[11px] uppercase tracking-wider text-primary font-semibold inline-flex items-center gap-1.5">
          <Target className="size-3.5" /> Цель
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        {(
          [
            ["Бюджет", state.scales.budget],
            ["Сроки", state.scales.timeline],
            ["Мотивация", state.scales.morale],
          ] as const
        ).map(([l, v]) => (
          <div key={l} className="rounded-lg border bg-card p-2.5">
            <div
              className={cn("text-xl font-bold tabular-nums", v <= 25 ? "text-destructive" : "")}
            >
              {v}
            </div>
  ...
  return (
    <div className="p-5 sm:p-6">
        ...
        <div className="mt-4 grid gap-2.5 sm:grid-cols-2">
          {step.docs.map((d, i) => (
            <div key={i} className="rounded-lg border bg-secondary/40 p-3">
              <div className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-primary uppercase tracking-wide">
      ...
      <div className="mt-5 text-sm font-semibold">
        {step.interaction ? "Задание:" : "Твоё решение:"}
      </div>
      <div className="mt-2">
        {step.interaction ? (
          <InteractionView
          ...
          <ChoiceList choices={step.choices} onChoose={onChoose} metricLabel={metricLabel} />
        )}
      </div>
    </div>
  );
    ...
    <div className="bg-[oklch(0.13_0.02_255)] text-white">
      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          <div className="size-11 shrink-0 rounded-full bg-white/10 grid place-items-center text-2xl">
            {step.avatar ?? "📱"}
          </div>
          <div className="min-w-0">
            <div className="text-sm font-semibold inline-flex items-center gap-1.5">
              {step.from} <Icon className="size-3.5 text-emerald-400" />
            </div>
            <div className="text-[12px] text-white/50">{step.role}</div>
          </div>
        </div>
        <div className="mt-3 rounded-2xl rounded-tl-sm bg-white/10 px-4 py-3 text-[14px] leading-relaxed">
          {step.situation}
        </div>
    ...
    <div className="bg-[oklch(0.12_0.02_255)] text-white">
      <div className="px-4 py-2 bg-black/40 flex items-center justify-between text-[11px] text-white/60">
        <span className="inline-flex items-center gap-1.5">
          <MonitorPlay className="size-3.5 text-primary" /> Zoom ·{" "}
          {step.meeting?.topic ?? step.title}
        </span>
        <span className="inline-flex items-center gap-1 text-emerald-400">
          <span className="size-1.5 rounded-full bg-emerald-400 animate-pulse" /> в эфире
        </span>
      </div>
      <div
        className={cn("grid gap-2 p-3", participants.length > 1 ? "grid-cols-2" : "grid-cols-1")}
      >
        {participants.map((p) => (
          <div
            key={p.name}
            className="relative rounded-lg bg-gradient-to-b from-white/10 to-white/5 border border-white/10 aspect-video grid place-items-center"
          >
            <div className="text-center">
            ...
            {p.mood && (
              <span className="absolute bottom-1.5 left-1.5 text-[10px] px-1.5 py-0.5 rounded bg-black/50 text-white/70">
                {p.mood}
              </span>
            )}
          </div>
      ...
      <div className="px-4 pb-4">
        <div className="rounded-xl bg-white/[0.04] border border-white/10 px-4 py-3 text-[14px] leading-relaxed">
          {step.situation}
        </div>
        <div className="mt-3 text-[11px] uppercase tracking-wider text-white/50 font-semibold">
          Твоя реплика — выбери тон:
        </div>
        <div className="mt-2 [&_button]:bg-white/[0.04] [&_button]:border-white/15 [&_button]:text-white [&_button:hover]:bg-white/10">
          {step.interaction ? (
            ...
            <ChoiceList choices={step.choices} onChoose={onChoose} zoom metricLabel={metricLabel} />
          )}
        </div>
      </div>
    </div>
...
function EndCard({
  ...
  const fatalStep = state.fatalStepId
    ? mission.steps.find((s) => s.id === state.fatalStepId)
    : undefined;
  return (
    <div className="w-full max-w-lg rounded-2xl bg-black/60 backdrop-blur-xl border border-white/15 shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-300">
      <div
        className={cn(
          ...
          won ? "bg-emerald-500/20 text-emerald-300" : "bg-destructive/20 text-destructive",
        )}
      >
        {won ? <Trophy className="size-3.5" /> : <Skull className="size-3.5" />}
        {won ? "Миссия выполнена" : "Проект провален"}
      </div>
      <h2 className="mt-3 text-2xl font-bold">{won ? "Цель достигнута! 🎉" : "Game Over"}</h2>
      ...
      {!won && fatalStep && (
        <div className="mt-4 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm">
          ...
          <div className="mt-0.5 text-white/80">
            {fatalStep.phase} · {fatalStep.title}
          </div>
        </div>
      )}
      ...
      <div className="mt-4 grid grid-cols-4 gap-2 text-center">
        {(
          [
            ["Метрика", `${state.metric}${mission.metric.unit}`],
            ["Бюджет", state.scales.budget],
            ["Сроки", state.scales.timeline],
            ["Команда", state.scales.morale],
          ] as const
        ).map(([l, v]) => (
    ...
    <div className="fixed inset-0 z-50 grid place-items-center px-4 py-6">
      ...
      <div
        className={cn(
          ...
          narrow ? "max-w-[360px]" : wide ? "max-w-2xl" : "max-w-md",
        )}
      >
