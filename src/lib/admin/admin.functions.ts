  ...
  .handler(async ({ context }): Promise<AdminAnalytics> => {
    ...
    const [{ data: attemptsRaw }, { data: progressRaw }, { data: appealsAgg }] = await Promise.all([
      supabaseAdmin
        .from("task_attempts")
        .select(
          "user_id, lesson_id, task_type, attempt_no, status, score, created_at, override_status",
        ),
      supabaseAdmin
    ...
    const startedSet = new Set(
      progress.filter((p) => p.lesson_id === firstId).map((p) => p.user_id),
    );
    const completedLastSet = new Set(
      progress
        .filter((p) => p.lesson_id === lastId && p.status === "completed")
        .map((p) => p.user_id),
    );
    const retention = startedSet.size
      ? Math.round((completedLastSet.size / startedSet.size) * 100)
      : 0;
    ...
    const lessons: LessonStat[] = LESSONS.map((lesson) => {
      ...
      const completed = new Set(lProg.filter((p) => p.status === "completed").map((p) => p.user_id))
        .size;
      ...
      const taskTypes: TaskTypeStat[] = types
        .map((t) =>
          statForTaskType(
            lAttempts.filter((a) => a.task_type === t),
            t,
          ),
        )
      ...
      const reached3Users = new Set(
        lAttempts.filter((a) => a.attempt_no >= 3).map((a) => a.user_id),
      );
      ...
      const avgAttempts = taskTypes.length
        ? Math.round((taskTypes.reduce((s, t) => s + t.avgAttempts, 0) / taskTypes.length) * 10) /
          10
        : 0;
    ...
    const taskTypeBreakdown: TaskTypeStat[] = TYPES.map((t) =>
      statForTaskType(
        attempts.filter((a) => a.task_type === t),
        t,
      ),
    ).filter((s) => s.attempts > 0);
...
function statForTaskType(rows: AttemptRow[], type: string): TaskTypeStat {
  ...
  rows.forEach((r) =>
    maxByUser.set(r.user_id, Math.max(maxByUser.get(r.user_id) ?? 0, r.attempt_no)),
  );
...
function buildFunnel(progress: ProgressRow[]): FunnelStep[] {
  return LESSONS.map((lesson) => {
    ...
    const completed = new Set(rows.filter((p) => p.status === "completed").map((p) => p.user_id))
      .size;
    return {
      number: lesson.number,
      lessonId: lesson.id,
  ...
  .handler(async ({ data, context }): Promise<CallLog[]> => {
    ...
    const [{ data: appealsRaw }, { data: attemptsRaw }] = await Promise.all([
      ...
      supabaseAdmin
        .from("task_attempts")
        .select(
          "id, user_id, lesson_id, task_type, status, score, ai_feedback, created_at, override_status, override_note",
        )
        .eq("task_type", "call")
        .order("created_at", { ascending: false }),
    ]);
    ...
    for (const t of (attemptsRaw ?? []) as any[]) {
      ...
      const feedback =
        typeof fb === "string"
          ? fb
          : fb && typeof fb === "object"
            ? (fb.feedback ?? JSON.stringify(fb))
            : null;
      const transcript =
        fb && typeof fb === "object" && Array.isArray(fb.transcript) ? fb.transcript : [];
      logs.push({
        id: t.id,
        source: "attempt",
        userId: t.user_id,
...
// ---------- allowed students (access control) ----------
/** Public check (no auth required) — used on login page to verify access */
export const isEmailAllowed = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rows, error } = await supabaseAdmin
      ...
      .maybeSingle();
    if (error) throw error;
    return { allowed: !!rows, student: rows ?? null };
  });
...
export const getAllowedStudents = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<AllowedStudent[]> => {
    await assertAdmin(context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data, error } = await supabaseAdmin
      .from("allowed_students")
...
export const addAllowedStudent = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => AddStudentInput.parse(d))
  .handler(async ({ data, context }) => {
    await assertAdmin(context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("allowed_students").insert({
      email: data.email.toLowerCase().trim(),
      display_name: data.displayName?.trim() || null,
      added_by: context.userId,
    });
    if (error) {
      if (error.code === "23505") throw new Error("Этот email уже добавлен");
      throw error;
    }
    return { ok: true };
  });

export const removeAllowedStudent = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    await assertAdmin(context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("allowed_students").delete().eq("id", data.id);
    if (error) throw error;
    return { ok: true };
  });
...
export const getStudents = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<StudentStat[]> => {
    ...
    const [
      ...
      supabaseAdmin.from("lesson_progress").select("user_id, status, updated_at"),
