...
/** Result applied when an interactive task is solved (correctly or not). */
...
/** Drag-to-order task: arrange items into the correct sequence. */
export interface OrderInteraction {
  kind: "order";
  prompt: string;
  /** Items shown shuffled to the player. */
...
/** Fill-in-the-blank task: pick the correct word to complete a sentence. */
export interface FillInteraction {
  kind: "fill";
  prompt: string;
  before: string;
  after: string;
...
/** Any interactive mini-task that replaces the plain choice list on a step. */
export type Interaction = OrderInteraction | FillInteraction;
...
export interface Step {
  ...
  /** Optional interactive task (drag-to-order / fill-in-the-blank). If set, it replaces the plain choice list. */
  interaction?: Interaction;

  /** The options the player can choose from (used when `interaction` is not set). */
  choices: Choice[];
}
